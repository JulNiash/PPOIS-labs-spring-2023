...
BACKGROUND_PICTURE = pygame.transform.scale(pygame.image.load(os.path.join("assets", "new_space.png")), (WIDTH, HEIGHT))
MENU_BACKGROUND = pygame.transform.scale(pygame.image.load(os.path.join("assets", "logo1.jpg")), (WIDTH, HEIGHT))
...
class Player(Entity):
    ...
    def healthbar(self, window):
        pygame.draw.rect(window, (255, 0, 0),
                         (self.x, self.y + self.entity_img.get_height() + 10, self.entity_img.get_width(), 10))
        pygame.draw.rect(window, (0, 255, 0),
                         (self.x, self.y + self.entity_img.get_height() + 10,
                                               self.entity_img.get_width() * (self.health / self.max_health), 10))
...
class Enemy(Entity):
    ...
    def __init__(self, x, y, color, health=10):  # "blue","green","yellow"
        super().__init__(x, y, health)
...
class Button:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.inactive_clr = (255, 200, 0)
        self.active_clr = (255, 200, 0)
...
def main():
    ...
    def window_redraw():
        ...
        points_label = points_font.render(f"Points:{player.points}", 1, (255, 255, 0))

        GAME_WINDOW.blit(points_label, (WIDTH / 4 - points_label.get_width(), 15))
        ...
        if len(enemies) == 0:
            ...
            for i in range(wave_length):
                enemy = Enemy(random.randrange(50, WIDTH - 100), random.randrange(-1500, -100), random.choice(
                    ["blue", "bolotniy", "yellow", "aquamarin", "orange", "pink", "purple", "red", "telecniy"]))
...
def show_menu():
    ...
    while show:
        ...
        start_button.draw(350, 400, "Начать игру", game)
        quit_button.draw(350, 500, "Выйти из игры", quit_from_game)
        pygame.display.update()
